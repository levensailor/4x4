---
name: Swift Design Requirements
overview: Define agent-ready requirements for preserving the Logic Pro.Guru SwiftUI app look and feel while extending or rebuilding the app.
todos:
  - id: preserve-brand-shell
    content: Preserve the Logic Pro.Guru brand, NavigationSplitView shell, home page, and chapter grouping.
    status: pending
  - id: reuse-section-system
    content: Render content through existing reusable section views and optional table layouts.
    status: pending
  - id: maintain-content-contract
    content: Keep the shared content bundle, offline cache, seed content, and tolerant decoding model intact.
    status: pending
  - id: verify-design-parity
    content: Verify representative screens for layout, readability, accessibility, and content compatibility.
    status: pending
isProject: false
---
...
- Use native SwiftUI materials and system colors rather than custom heavy styling:
  ...
  - `.thinMaterial` for soft grouped content surfaces.a
